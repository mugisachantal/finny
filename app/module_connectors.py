...
def get_loan_recommendations_preview(
    ...
    **_kwargs,
    ...
    try:
        response = requests.post(_recommendation_endpoint_url(), json=payload, timeout=6)
            ...
            matches.append(
                {
                    ...
                    "installmentEstimate": int(item.get("totalRepayment", 0) / max(int(tenureDays or 30), 1)),
        ...
        return {"tenureAssumedDays": int(tenureDays or 30), "matches": matches, "comparisonSummary": comparison_summary}
    except Exception:
        return {"tenureAssumedDays": int(tenureDays or 30), "matches": [], "comparisonSummary": None}
