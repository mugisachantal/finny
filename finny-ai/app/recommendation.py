...
def score_lenders(
    ...
    for lender in lender_list:
        ...
        min_tenure = int(
            lender.get("minTenureDays", lender.get("min_tenure_days", 0)) or 0
        )
        max_tenure = int(
            lender.get("maxTenureDays", lender.get("max_tenure_days", 0)) or 0
        )
    ...
    for lender in qualifying:
        total_repayment = float(
            lender.get(
                "totalRepayment", lender.get("total_repayment", requested_amount)
            )
            or requested_amount
        )
        ...
        cost_score = (
            100 - min(100, ((total_repayment / requested_amount - 1) * 100))
            if requested_amount
            else 100
        )
        upheld_complaints = int(
            lender.get("upheldComplaints", lender.get("upheld_complaints", 0)) or 0
        )
        licensed = bool(
            lender.get("licensedByUMRA", lender.get("licensed_by_umra", True))
        )
